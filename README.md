...
**Mott** (*мотт*, чеч. *язык*) — хобби-язык со словарём из чеченского: `xilit` вместо `let`, `nagah sanna` вместо `if`, `cqachunna` вместо `while`. Под капотом — транспиляция в C и сборка через `clang`, на выходе — быстрый нативный бинарник.
...
- 📖 **[Учебник](docs/book/README.md)** — пошаговое введение в язык, от "Hello, World" до собственного проекта. 9 глав + справочник.
