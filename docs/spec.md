...
`fnc`, `xilit`, `yuxadalo`, `yazde`, `nagah`, `sanna`, `khi`, `cqachunna`, `baqderg`, `xarco`, `a`, `ya`, `terah`, `bool`, `deshnash`, `daqosh`.
...
if_stmt        = "nagah sanna" "(" expr ")" block [ "khi" block ] ;
...
- `else if` — нужно вложенное `nagah sanna` внутри `khi`
